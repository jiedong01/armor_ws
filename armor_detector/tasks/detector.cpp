...
namespace auto_aim
{
  ...
  private:
      void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg)
      {
          try
          {
              // 使用cv_bridge将ROS图像消息转换为cv::Mat
              auto cv_image = cv_bridge::toCvShare(msg, "bgr8")->image;

              // 创建Detector对象并调用detect方法
              Detector detector;
              auto armors = detector.detect(cv_image);

              // 处理检测到的装甲板...
              // 这里可以添加代码来处理armors，例如发布结果或进行其他操作
          }
          catch (const cv_bridge::Exception& e)
          {
              RCLCPP_ERROR(this->get_logger(), "Could not convert image to OpenCV format: %s", e.what());
          }
      }
  ...
  {
    cv::dnn::Net net = cv::dnn::readNetFromONNX("resource/tiny_resnet.onnx");
    cv::Mat gray;
